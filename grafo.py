...
def generar_matriz(n, manual):
    ...
    if not manual:
        ...
        for i in range(1, n):
            for j in range(i):
                valor = simpledialog.askfloat(f"Valor de la arista ({i}, {j})", f"Ingrese el valor de la arista ({i}, {j}):", minvalue=0)
                if valor is None:  # Usuario cerró la ventana sin ingresar valor
                    valor = 0
...
def dibujar_matriz(matriz):
    n = len(matriz)
    fig, ax = plt.subplots()
    ax.matshow(matriz, cmap='viridis')
    
    for i in range(n):
        for j in range(n):
            ax.text(j, i, str(matriz[i, j]), va='center', ha='center', fontsize=10, color='w')
...
class Grafo:
    ...
    def crear_ventana(self):
        # ventana para mostrar grafo
        self.ventana = Tk()
        self.ventana.title("Grafo")
        self.ventana.figura, self.ventana.eje = plt.subplots()
        self.ventana.canvas = FigureCanvasTkAgg(self.ventana.figura, master=self.ventana)
        self.ventana.canvas_widget = self.ventana.canvas.get_tk_widget()
        self.ventana.canvas_widget.pack(side=TOP, fill=BOTH, expand=1)
    
    def mostrar_grafo(self, camino=None):
        self.crear_ventana()

        posiciones = nx.spring_layout(self.grafo)
        nx.draw_networkx_nodes(self.grafo, posiciones, ax=self.ventana.eje)
        nx.draw_networkx_edges(self.grafo, posiciones, ax=self.ventana.eje)
        if camino:
            nx.draw_networkx_edges(self.grafo, posiciones, edgelist=camino, edge_color='blue', width=2, ax=self.ventana.eje)
        etiquetas = {nodo: nodo for nodo in self.grafo.nodes()}
        nx.draw_networkx_labels(self.grafo, posiciones, etiquetas, ax=self.ventana.eje)
        etiquetas_aristas = nx.get_edge_attributes(self.grafo, 'weight')
        nx.draw_networkx_edge_labels(self.grafo, posiciones, edge_labels=etiquetas_aristas, ax=self.ventana.eje)
        
        self.ventana.eje.axis('off')
